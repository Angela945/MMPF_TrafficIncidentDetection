...
## This upper class defines the noise models and left boundary conditions
class Input(object):
        def __init__(self):
                self.noiseCTM=1.0
...
## This function describes the transition of regime variable
...
## specify NS or WS+number, the number is the value of lag (\Delta S in the paper).
...
#        smoothing='WS4'
        ...
        inputDensity=load('TrueDensity.npy')
        inputSpeed=load('RawSpeed'+str(headway)+'.npy')
...
## Here ends the smoothing algorithm
                ...
                Weight=initializeWeight(s.sample)
        save('Result'+str(headway)+smoothing+'StateEstimate',stateEstimate)
        save('Result'+str(headway)+smoothing+'ParameterEstimate',parameterEstimate)
...
plt.colorbar()
plt.savefig('Result'+str(headway)+smoothing+'Density.pdf',bbox_inches='tight')
plt.show()
plt.clf()
...
plt.clim(1.0, 3.0)
plt.colorbar()
plt.savefig('Result'+str(headway)+smoothing+'Parameter.pdf',bbox_inches='tight')
plt.show()
plt.clf()
