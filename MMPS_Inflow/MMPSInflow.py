...
## set the numerical simulation
        ...
        def setLeftGhost(self,counter):
                self.leftGhost=1000+random.normal(0,50)
...
#        smoothing='NS'
        ...
        inputDensity=load(str(flow)+'RawDensity'+str(headway)+'.npy')
        inputSpeed=load(str(flow)+'RawSpeed'+str(headway)+'.npy')
        ...
        gammaPos=zeros((s.timeStepNumber, s.sample))
...
## Here ends the smoothing algorithm
                ...
                gammaPos[counter,:]=s.vectorGamma[:,0,4]
                
        save(str(flow)+'Result'+str(headway)+smoothing+'StateEstimate',stateEstimate)
        save(str(flow)+'Result'+str(headway)+smoothing+'ParameterEstimate',parameterEstimate)
...
plt.colorbar()
plt.savefig(str(flow)+'Result'+str(headway)+smoothing+'Density.pdf',bbox_inches='tight')
plt.show()
plt.clf()
...
plt.clim(1.0, 3.0)
plt.colorbar()
plt.savefig(str(flow)+'Result'+str(headway)+smoothing+'Parameter.pdf',bbox_inches='tight')
plt.show()
plt.clf()

## Generate the posterior distribution of gamma

##plt.rc('xtick',labelsize=30)
##plt.rc('ytick',labelsize=30)
##plt.xticks([1,2,3])
##imgplot2=plt.hist(gammaPos[80,:], bins=3, range=(0.5,3.5),align='mid',rwidth=0.5,color='blue')
##plt.ylabel('Frequency',fontsize=30)
##plt.xlabel('Regime variable (lanes)',fontsize=30)
##plt.savefig('Gamma_cell4_T80.pdf',bbox_inches='tight')
##plt.show()
##plt.clf()
